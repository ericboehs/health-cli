...
module Health
  module Commands
    ...
    # *authorized* — not what is left. See `notes`.
    class Meds < Resource
      ...
      # "expired" is not an R4 MedicationRequest status and "completed or
      # stopped" leaves out the two that matter most to see: `cancelled` and
      # `entered-in-error`. Naming the boundary instead of the members says
      # the true thing in fewer words.
      def hidden_note
        return if @hidden.to_i.zero?

        @err.puts "#{@hidden} more on record (completed, stopped, cancelled or otherwise inactive) — " \
                  "see `health meds --all`."
      end
